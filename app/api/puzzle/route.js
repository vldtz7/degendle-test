...
// Enhanced with security headers and rate limiting
// ============================================================================
...
export async function GET(request) {
  ...
  // Response with only hint data - NEVER include answer
  const response = {
    hint: puzzle.hint,
    category: puzzle.category,
    dayNumber: puzzle.dayNumber,
    imageHint: puzzle.imageHint, // For preloading
    totalPuzzles: puzzles.length,
  };
  
  return Response.json(response, {
    headers: {
      // Cache for 5 minutes to reduce server load, but not too long
